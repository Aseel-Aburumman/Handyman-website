                        ...
                        <table class="table table-bordered">
                            ...
                            <tbody>
                                @forelse($Rhandymen as $handyman)
                                    <tr>
                                        <td>{{ $handyman->name }}</td>
                                        <td>{{ $handyman->gigs->where('status_id', 9)->count() }}</td>
                                        <!-- Assuming '9' is the 'Completed' status -->
                                        <td class="tableHide">{{ number_format($handyman->reviews->avg('rating'), 2) }}</td>
                                        <td>
                                            <a href="{{ route('admin.view_handyman', $handyman->id) }}"
                                                class="btn btn-info btn-sm">View</a>
                                            @if ($handyman->suspended)
                                                <form action="{{ route('admin.unsuspend_handyman', $handyman->id) }}"
                                                    method="POST" style="display:inline;">
                                                    @csrf
                                                    <button type="submit" class="btn btn-success btn-sm">Unsuspend</button>
                                                </form>
                                            @else
                                                <form action="{{ route('admin.suspend_handyman', $handyman->id) }}"
                                                    method="POST" style="display:inline;">
                                                    @csrf
                            ...
                            <tbody>
                                @foreach ($handymen as $handyman)
                                    <tr>
                                        ...
                                        <td class="tableHide">{{ number_format($handyman->reviews->avg('rating'), 2) }}</td>
                                        <td>
                                            <a href="{{ route('admin.view_handyman', $handyman->user->id) }}"
                                                class="btn btn-info btn-sm">View</a>
